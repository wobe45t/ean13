    ...
    const vector<XY> &points, const string &theOperator)
{
  ...
  for (int i = 0; i < n; i++)
  {
    out << points[i].mX << " " << points[i].mY << " " << (i == 0 ? OP_MOVETO : OP_LINETO) << "\n";
  }
  ...
  {
    out << points[0].mX << " " << points[0].mY << " " << OP_LINETO << "\n";
  }
    ...
    int x, int y, int width, int height, const string &theOperator)
{
  ...
  out << x << " " << y << " " << width << " " << height << " " << OP_RECTANGLE << " " << theOperator << "\n";
    ...
    const string &theOperator)
{
  ...
  out << (red / 255.0) << " " << (green / 255.0) << " " << (blue / 255.0) << " " << theOperator << "\n";
...
PDF::PDF(int width, int height) : mWidth(width),
                                  mHeight(height),
                                  mCurrentFont(NONE),
                                  mCurrentFontSize(0)
{
...
string PDF::toString()
{
  ...
  for (int i = 0; i < nEntries; i++)
  {
    ...
    for (int y = 0; y < height; y++)
    {
      for (int x = 0; x < width; x++)
      {
        ...
        tmp << hex << uppercase << setfill('0') << setw(2) << r << setw(2) << g << setw(2) << b;
  ...
  if (nEntries)
  {
    ...
    for (int i = 0; i < nEntries; i++)
    {
      out << "      " << mEntries[i].mInfo.mName << " " << mEntries[i].mInfo.mObjectID << " 0 R"
          << "\n";
    }
    ...
    const ImageInfo &info, int x, int y, double xScale, double yScale)
{
  ...
  out << OP_SAVE << " " << width << " " << 0 << " " << 0 << " " << height << " " << x << " " << y << " " << OP_CONCAT_MATRIX << " " << info.mName << " " << OP_DO << " " << OP_RESTORE << "\n";
